...
<html>
    ...
    <body>
        ...
        <div >
            <div class="span3">
                <a href="export.php?type=read_applications" class="btn btn-large btn-danger">EXPORT As CSV</a> 
            </div>
            <h1><center><i>APPLICATIONS</i></center></h1>
            <br>
            <?php
            ...
            $seasonNameField = new \Kendo\Data\DataSourceSchemaModelField('name');
            $seasonNameField->type('string')
                    ->editable(false)
                    ->nullable(true);
            $AppFirstNameField = new \Kendo\Data\DataSourceSchemaModelField('first_names');
            $AppFirstNameField->type('string')
                    ->editable(false)
                    ->nullable(true);
            $AppMiddleNameField = new \Kendo\Data\DataSourceSchemaModelField('middle_names');
            $AppMiddleNameField->type('string')
                    ->editable(false)
                    ->nullable(true);
            $AppLastNameField = new \Kendo\Data\DataSourceSchemaModelField('last_names');
            $AppLastNameField->type('string')
                    ->editable(false)
                    ->nullable(true);
            $paymentsFeeField = new \Kendo\Data\DataSourceSchemaModelField('fee');
            $paymentsFeeField->type('string')
                    ->editable(false)
                    ->nullable(true);
            $paymentsStatusField = new \Kendo\Data\DataSourceSchemaModelField('fee');
            $paymentsStatusField->type('string')
                    ->editable(false)
                    ->nullable(true);
            $faApplStatusField = new \Kendo\Data\DataSourceSchemaModelField('status ');
            ...
            $seasonNameField->field('name')
                    ->width(40)
                    ->title('Season');

            $ApplNameField = new \Kendo\UI\GridColumn();
            $ApplNameField->field('first_names', 'last_names', 'middle_names')
                    ->width(50)
                    ->title('Applicant Name');

            $paymentsFeeField = new \Kendo\UI\GridColumn();
            $paymentsFeeField->field('fee')
                    ->title('Payment Fee')
                    ->width(50);
            $paymentsStatusField = new \kendo\UI\GridColumn();
            $paymentsStatusField->field('status')
                    ->title('Payment Status')
                    ->width(50);

            $faApplStatusField = new \kendo\UI\GridColumn();
            $faApplStatusField->field('status')
